...
export default function Page() {
  ...
  // Detect when the page is accessed directly in auth failure cases
  useEffect(() => {
    // Set a timeout to show the bypass option after a few seconds
    const timeoutId = setTimeout(() => {
      if (isLoadingUser) {
        console.log("Auth check is taking too long, showing bypass option");
  ...
  // Modified fetch user effect
  useEffect(() => {
    async function fetchUser() {
      try {
        // If we're bypassing auth check, don't actually check
        if (BYPASS_AUTH_CHECK) {
          console.log("Bypassing auth check as requested");
          setUser({ id: 'bypassed-auth-check', email: 'user@example.com' });
        ...
        // If no user is found but we're on this page, we might have a session detection issue
        if (!currentUser) {
          console.log("No user detected, but access wasn't blocked by middleware");
  ...
  // If still loading user or auth check failed, show appropriate UI
  if (isLoadingUser) {
    return (
      <>
        <Header />
          ...
          {showBypassOption && (
            <div className="mt-4">
              ...
              <p className="mt-2 text-sm text-gray-500">
                If you're experiencing login issues, this will let you use the page anyway.
              </p>
            </div>
          )}
        </div>
  ...
  if (authCheckFailed && !BYPASS_AUTH_CHECK) {
    return (
      <>
        <Header />
        <div className="container mx-auto px-4 py-16 text-center">
          <h1 className="text-2xl font-bold mb-4">Authentication Issue Detected</h1>
          <p className="mb-6">We're having trouble confirming your login status.</p>
          
          <div className="flex flex-col gap-4 items-center justify-center">
            <button 
              onClick={() => router.push('/auth?redirectTo=/make-date')}
